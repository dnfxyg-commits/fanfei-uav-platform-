...
const NewsView: React.FC = () => {
  ...
  const categories = ['全部', '公司动态', '技术创新', '市场合作', '行业观点'];
              ...
              {filteredNews.map((item) => (
                <article key={item.id} className="group flex flex-col bg-white rounded-3xl overflow-hidden border border-slate-100 hover:border-blue-200 hover:shadow-2xl transition-all duration-500">
                  ...
                  <div className="p-8 flex-grow flex flex-col">
                    ...
                    <button className="mt-auto flex items-center gap-2 text-blue-600 font-bold text-sm group-hover:gap-3 transition-all">
                      阅读全文
                      <ArrowRight size={16} />
                    </button>
                  </div>
